    ...
    def process_updates(self):
        ...
        try:
            ...
            if self.pending_updates:
                logger.info(f"检测到以下文件变化:\n" + "\n".join(f"- {os.path.basename(f)}" for f in self.pending_updates))
...
class EnhancedReadmeUpdater:
    def __init__(self, project_root: str):
        ...
        self.config_path = os.path.join(project_root, '.github', 'config', 'readme_config.yml')
        self.module_pattern = re.compile(r'<!-- BEGIN_MODULES -->\n(.*?)\n<!-- END_MODULES -->', re.DOTALL)
    ...
    def _collect_module_info(self) -> Dict:
        """收集模块信息"""
        categorized_modules = {category: {} for category in self.config.get('categories', {})}
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if not node.name.startswith('_') or self.config.get('doc_options', {}).get('include_private_methods', False):
    ...
    def _parse_class_info(self, node: ast.ClassDef) -> Dict:
        ...
        for item in node.body:
            if isinstance(item, ast.FunctionDef):
                if not item.name.startswith('_') or self.config.get('doc_options', {}).get('include_private_methods', False):
    ...
    def _format_doc_section(self, category: str, modules: Dict) -> str:
        ...
        if self.config.get('badges', {}).get('show', True):
            ...
            if 'status' in badge_types:
                badges.append("![Status](https://img.shields.io/badge/status-active-success)")
            if 'version' in badge_types:
                badges.append("![Version](https://img.shields.io/badge/version-1.0.0-blue)")
            if 'coverage' in badge_types:
                badges.append("![Coverage](https://img.shields.io/badge/coverage-80%25-yellowgreen)")
            if 'last_update' in badge_types:
                badges.append(f"![LastUpdate](https://img.shields.io/badge/last_update-{datetime.now().strftime('%Y---%m---%d')}-informational)")
        ...
        for module_path, info in sorted(modules.items()):
            ...
            if info['classes']:
                ...
                for cls in info['classes']:
                    # 使用模板生成类文档
                    class_template = self.config.get('templates', {}).get('class_header', '')
                    ...
                    if cls['methods']:
                        ...
                        for method in cls['methods']:
                            content.append(f"- `{method['name']}`：{method['doc'].split('\n')[0]}")
                            if method['parameters'] or method['returns'] or method['examples']:
                                content.append("  <details>")
                                content.append("  <summary>详细信息</summary>\n")
                                if method['parameters']:
                                    content.append("  **参数：**\n  ```python")
                                    content.extend(f"  {param}" for param in method['parameters'])
                                    content.append("  ```\n")
                                if method['returns']:
                                    content.append(f"  **返回值：** `{method['returns']}`\n")
                                if method['examples']:
                                    content.append("  **示例：**\n  ```python")
                                    content.extend(f"  {example}" for example in method['examples'])
                                    content.append("  ```")
                                content.append("  </details>")
                        content.append("</details>\n")
            ...
            if info['functions']:
                ...
                for func in info['functions']:
                    # 使用模板生成函数文档
                    function_template = self.config.get('templates', {}).get('function_header', '')
                    ...
                    func_content = function_template.format(
                        ...
                        parameters='\n'.join(func['parameters']) if func['parameters'] else '无',
                        returns=func['returns'] if func['returns'] else '无',
                        example='\n'.join(func['examples']) if func['examples'] else '暂无示例',
                        badges=' '.join(badges) if badges else ''
                    )
                    content.append(f"\n{func_content}")
