...
def test_counter_creation() -> None:
    """测试创建计数器指标."""
    manager = MetricsManager()
    counter = manager.counter("test_counter", "Test counter", ["label1", "label2"])
...
def test_gauge_creation() -> None:
    """测试创建仪表盘指标."""
    manager = MetricsManager()
    gauge = manager.gauge("test_gauge", "Test gauge", ["label1"])
...
def test_histogram_creation() -> None:
    ...
    histogram = manager.histogram(
        "test_histogram", "Test histogram", ["label1"], buckets=[0.1, 0.5, 1.0]
    )
...
def test_summary_creation() -> None:
    """测试创建摘要指标."""
    manager = MetricsManager()
    summary = manager.summary("test_summary", "Test summary", ["label1"])
...
def test_metrics_functionality() -> None:
    ...
    # 测试直方图
    histogram = manager.histogram(
        "test_histogram", "Test histogram", buckets=[0.1, 1.0, 10.0]
    )
